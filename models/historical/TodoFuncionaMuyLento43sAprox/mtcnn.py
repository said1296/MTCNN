...
from tensorflow.keras.layers import Input, Conv2D, PReLU, MaxPooling2D, Reshape, Dense, GlobalMaxPool2D, Softmax, Flatten
...
def pnetModel(inputShape=(None, None, 3)):
    ...
    pnetBoxRegression = Flatten()(pnetLayer)
    pnetBoxRegression = Dense(32)(pnetBoxRegression)
    pnetBoxRegression = PReLU()(pnetBoxRegression)
    pnetBoxRegressionOutput = Dense(4, activation='sigmoid', name='boxRegression')(pnetBoxRegression)
...
def trainPnet(type='pnetClassification', loadModelFlag=True, tfrecordsDir=os.getcwd()+'/dataset/tfrecords/', size=24, inputShape=(None, None, 3), batchSize=500, validationSplit=0.1):
    ...
    if(type=='pnetClassification'):
        ...
        lossWeights={'classification': 1, 'boxRegression': 0}
    ...
    pnet.fit(trainIterator(parsedTrainDataset, size), validation_data=validationIterator(parsedValidationDataset, size), epochs=1, steps_per_epoch=trainBatches, validation_steps=validationBatches, verbose=1, shuffle=True)
    pnet.save(modelPath)
